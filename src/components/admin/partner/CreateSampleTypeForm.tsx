...
import { createSampleType } from "@/actions/partner/sampleTypes";
import { SubItemFormState } from "@/actions/partner/PartnerFormState";
import FormButton from "@/components/common/formButton/formButton";

export function CreateSampleTypeForm() {
  const [formState, action] = useFormState<SubItemFormState, FormData>(createSampleType, {});
  ...
  return (
    <form action={action} className="flex flex-wrap items-center gap-2">
      <input name="name" type="text" placeholder="Название типа образца" value={name} onChange={(e) => setName(e.target.value)} className="admin-form-input text-sm w-56" />

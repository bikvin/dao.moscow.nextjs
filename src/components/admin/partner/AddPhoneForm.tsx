...
export function AddPhoneForm({ partnerId }: { partnerId: string }) {
  ...
  useEffect(() => {
    if (formState.success) { setPhone(""); setNotes(""); }
  }, [formState.success]);

  return (
    <CollapsibleAddSection label="Добавить телефон" success={!!formState.success}>
      <form action={action} className="flex flex-wrap items-center gap-2">
        <input name="phone" type="text" placeholder="Телефон" value={phone} onChange={(e) => setPhone(e.target.value)} className="admin-form-input text-sm w-44" />
        <input name="notes" type="text" placeholder="Примечание (чей номер)" value={notes} onChange={(e) => setNotes(e.target.value)} className="admin-form-input text-sm w-52" />

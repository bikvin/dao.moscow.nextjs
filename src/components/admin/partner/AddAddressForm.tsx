  ...
  return (
    <CollapsibleAddSection label="Добавить адрес" success={!!formState.success}>
      <form action={action} className="flex flex-wrap items-center gap-2">
        <select name="type" value={type} onChange={(e) => setType(e.target.value as AddressTypeEnum)} className="admin-form-input text-sm w-32">
          {Object.values(AddressTypeEnum).map((t) => (
            <option key={t} value={t}>{TYPE_LABELS[t]}</option>
          ))}
        </select>
        <input name="address" type="text" placeholder="Адрес" value={address} onChange={(e) => setAddress(e.target.value)} className="admin-form-input text-sm w-72" />

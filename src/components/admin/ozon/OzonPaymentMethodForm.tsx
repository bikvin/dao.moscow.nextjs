...
import { saveOzonPaymentMethod } from "@/actions/ozon/saveOzonPaymentMethod";

type PaymentMethodOption = { id: string; name: string };

// Lets the user select which payment method represents marketplace payments from Ozon.
// The selected method is auto-assigned to imported orders and hides the paid toggle in order cards.
export function OzonPaymentMethodForm({
  paymentMethods,
  ...
  const [formState, action] = useFormState(saveOzonPaymentMethod, {});

...
export function WarehouseProductRow({
  ...
  return (
    <div>
      {/* Product row */}
      <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr_2fr] gap-2 py-2 px-3 rounded-md bg-slate-100 font-medium items-center">
        <div>{product.sku}</div>
      ...
      {multiVariant &&
        variants.map((variant) => {
          ...
          return (
            <div
              key={variant.id}
              className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr_2fr] gap-2 py-1 px-3 pl-8 text-sm text-slate-600 items-center"
            >
              <div>{variant.variantName}</div>
              <div className="text-center">{variant.warehouseQuantity}</div>
              <div className="text-center">{area(variant.warehouseQuantity, product.length_mm, product.width_mm)}</div>
              <div className="text-center text-green-600">{variant.availableQuantity}</div>
              <div className="text-center text-green-600">{area(variant.availableQuantity, product.length_mm, product.width_mm)}</div>
              <div className="text-center text-amber-500">{reserved}</div>
              <div className="text-center text-amber-500">{area(reserved, product.length_mm, product.width_mm)}</div>

    ...
    if (result.data.dealerPrice && result.data.dealerCurrency) {
      ...
      priceUpserts.push(
        db.price.upsert({
          ...
          create: { productId: result.data.id, type: PriceTypeEnum.DEALER, priceInCents: Math.round(Number(result.data.dealerPrice) * 100), currency: result.data.dealerCurrency as CurrencyEnum, unit: dealerUnit, quantity: dealerQuantity },
        })
      );
    }
    if (result.data.retailPrice && result.data.retailCurrency) {
      ...
      priceUpserts.push(
        db.price.upsert({
          ...
          create: { productId: result.data.id, type: PriceTypeEnum.RETAIL, priceInCents: Math.round(Number(result.data.retailPrice) * 100), currency: result.data.retailCurrency as CurrencyEnum, unit: retailUnit, quantity: retailQuantity },
        })
      );
    }

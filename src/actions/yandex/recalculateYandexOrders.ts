...
// Fetches financial stats from Yandex for a list of order IDs.
...
// any orders that are now settled. Returns success message or form errors.
export async function recalculateYandexOrders(
  ...
  try {
    ...
    if (unsettled.length === 0) return { success: { message: "Нет заказов для пересчёта" } };

    // Determine date range covering all unsettled orders
    ...
    const stats = await fetchStats(
      [...unsettledYandexIds].map((id) => parseInt(id))
    );

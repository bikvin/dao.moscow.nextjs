...
export default async function OzonPriceSyncHistoryPage({
  searchParams,
}: {
  ...
  const [logs, total] = await Promise.all([
    db.ozonPriceSyncLog.findMany({
      orderBy: { createdAt: "desc" },
      skip: (currentPage - 1) * PAGE_SIZE,
      take: PAGE_SIZE,
    }),
    db.ozonPriceSyncLog.count(),
  ]);
    ...
    <>
      ...
      <div className="max-w-screen-lg mx-auto">
        <div className="w-[90%] md:w-2/3 mx-auto">
          <h1 className="admin-form-header mt-10">История синхронизаций цен Ozon</h1>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span
                      className={`text-sm font-medium ${
                        log.status === OzonSyncStatusEnum.SUCCESS
                          ? "text-emerald-600"
                          : "text-red-600"
                      }`}
                    >
                      {log.status === OzonSyncStatusEnum.SUCCESS ? "Успешно" : "Ошибка"}
                    </span>
          ...
          <Pagination
            ...
            basePath="/admin/ozon/price-sync-history"

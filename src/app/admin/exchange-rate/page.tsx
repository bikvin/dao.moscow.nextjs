...
export default async function ExchangeRatePage() {
  const [officialRateSetting, rmbRateSetting, mainRateSetting, modeSetting, markupSetting] = await Promise.all([
      ...
      <div className="max-w-screen-lg mx-auto">
        <div className="w-[90%] md:w-2/3 mx-auto">
          ...
          <div className="border rounded-md p-4 mt-4 shadow-main">
            <h2 className="text-lg font-medium mb-3">Официальный курс ЦБ РФ (CNY)</h2>
            {rmbRate !== null ? (
              <div className="text-sm flex flex-wrap gap-x-4 gap-y-1">
                <span className="text-slate-700 font-medium">
                  1 CNY = {rmbRate.toFixed(4)} ₽
                </span>
                <span className="text-slate-400">
                  Обновлено: {new Date(rmbRateSetting!.updatedAt).toLocaleString("ru-RU")} UTC
                </span>
              </div>
            ) : (
              <p className="text-sm text-slate-400">Данные ещё не загружены</p>

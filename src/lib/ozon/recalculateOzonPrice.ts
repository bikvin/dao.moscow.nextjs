...
export async function recalculateOzonPrice(productId: string): Promise<void> {
  const [product, exchangeRateSetting, markupSetting, divisorSetting] = await Promise.all([
    db.product.findUnique({
      ...
      select: {
        ...
        ozonMappings: { select: { divisor: true, priceMarkup: true } },
      },
    }),
  ...
  if (!product || product.ozonMappings.length === 0) return;
  ...
  const mapping = product.ozonMappings[0];
  ...
  const priceOzon = priceUnitRub * effectiveDivisor * multiplier;
  const priceInCents = Math.round(priceOzon * 100);

  await db.price.upsert({
    where: { productId_type: { productId, type: PriceTypeEnum.OZON } },
    update: { priceInCents, currency: CurrencyEnum.RUB, unit: PriceUnitEnum.ITEM, quantity: effectiveDivisor },
    create: { productId, type: PriceTypeEnum.OZON, priceInCents, currency: CurrencyEnum.RUB, unit: PriceUnitEnum.ITEM, quantity: effectiveDivisor },
  });
}
